...
import { DOCTOR_NAV, DOCTOR_PAGE_TITLES } from '../constants/doctorNav'
...
// ── Sidebar ────────────────────────────────────────────────────────────────
function DoctorSidebar({ collapsed, onNavClick }) {
<<<<<<< HEAD
...
=======
            <span className="shrink-0 relative">
              {item.icon}
              {item.href === '/doctor/chat' && chatUnread > 0 && (
            ...
            {!collapsed && item.href === '/doctor/chat' && chatUnread > 0 && (
...
// ── Header ─────────────────────────────────────────────────────────────────
function DoctorHeader({ onToggleSidebar }) {
  const { pathname } = useLocation()
  const navigate     = useNavigate()
  const title        = DOCTOR_PAGE_TITLES[pathname] ?? 'Dashboard'
    ...
    <header className="flex items-center justify-between shrink-0 sticky top-0 z-30 bg-white border-b border-[#E2E8F0]"
      ...
      <div className="flex items-center gap-1">
        ...
        <div ref={notifRef} className="relative">
          <button onClick={() => setNotifOpen(v => !v)} className={`${iconBtn} relative`}>
            <Bell size={19} />
          </button>
          {notifOpen && (
        ...
        <div ref={profileRef} className="relative">
          <button onClick={() => setProfileOpen(v => !v)}
            ...
            <div className="header-profile-text text-left leading-tight">
              ...
              <p className="m-0 text-xs text-[#64748B]">{user?.specialization ?? 'Doctor'}</p>
            </div>
          ...
          {profileOpen && (
            <div className={dropdown} style={{ minWidth: 180 }}>
              <a href="/doctor/settings" className={menuItem}><User size={14} /> My Profile</a>
              <a href="/doctor/settings" className={menuItem}><Settings size={14} /> Settings</a>
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#F8FAFB]">
      ...
      <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
        <DoctorHeader onToggleSidebar={toggleSidebar} />
