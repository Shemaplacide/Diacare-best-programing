...
import { Calendar, List, Plus, ChevronLeft, ChevronRight, CalendarClock } from 'lucide-react'
import { toast } from 'react-toastify'
import Modal from '../../components/ui/Modal'
import InputField from '../../components/ui/InputField'
import Button from '../../components/ui/Button'
import { StatusBadge, APPOINTMENT_STATUS } from '../../constants/status'
import { getMyAppointments, bookAppointment, reschedule as rescheduleApi, cancelAppointment } from '../../api/appointments'
import { getAllDoctors } from '../../api/doctor'

const APPT_TYPES = ['Routine Check-up', 'Glucose Review', 'Insulin Adjustment', 'Diet Consultation', 'Lab Follow-up', 'Emergency Visit', 'Foot Care', 'Eye Screening']
...
function fmt(date) { return date instanceof Date ? date.toISOString().split('T')[0] : date?.split('T')[0] }
...
export default function PatientAppointments() {
  ...
  const [form,           setForm]           = useState({ doctorPublicId: '', type: '', date: '', time: '', duration: '30', notes: '' })
  const [saving,         setSaving]         = useState(false)
  const handle = f => e => setForm(p => ({ ...p, [f]: e.target.value }))

  const load = () => {
    setLoading(true)
    Promise.all([getMyAppointments(), getAllDoctors()])
      .then(([apptRes, docRes]) => {
  ...
  const validateDateTime = (date, time) => {
    ...
    if (day === 0 || day === 6) { toast.error('Weekdays only (Mon–Fri)'); return false }
    const hour = parseInt(time.split(':')[0])
    if (hour < 9 || hour >= 18) { toast.error('Must be between 9:00 AM and 6:00 PM'); return false }
    return true
  }
  ...
  const reschedule = async () => {
    ...
    if (!validateDateTime(rescheduleForm.date, rescheduleForm.time)) return
    setSaving(true)
    try {
      const newDateTime = `${rescheduleForm.date}T${rescheduleForm.time}:00`
      await rescheduleApi(selected.id, newDateTime)
  ...
  const book = async () => {
    if (!form.doctorPublicId) { toast.error('Please select a doctor'); return }
    if (!form.type || !form.date || !form.time) { toast.error('Fill in all required fields'); return }
    if (!validateDateTime(form.date, form.time)) return
    setSaving(true)
    try {
      await bookAppointment({
        appointmentDate: `${form.date}T${form.time}:00`,
        notes: form.notes || form.type,
        doctorPublicId: form.doctorPublicId,
      })
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-[#64748B] mt-1 m-0">{appts.filter(a => a.status === 'PENDING' || a.status === 'CONFIRMED').length} upcoming</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button size="md" onClick={() => { setForm({ doctorPublicId: '', type: '', date: '', time: '', duration: '30', notes: '' }); setModalOpen(true) }}>
            <Plus size={15} /> Book
          </Button>
        </div>
      </div>
                      ...
                      {slotAppts.map(a => {
                        ...
                        return (
                          <div key={a.id} onClick={() => { setSelected(a); setDetailOpen(true) }}
                            ...
                            <p className="m-0 text-xs text-[#64748B] truncate">{a.doctor?.user?.username ?? a.notes ?? 'Appointment'}</p>
                          </div>
                        )
                      })}
                    </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          {['Upcoming', 'Past'].map(group => {
            const groupAppts = appts.filter(a => {
              ...
              return group === 'Upcoming' ? !isPast : isPast
            })
                    ...
                    return (
                      <div key={a.id} onClick={() => { setSelected(a); setDetailOpen(true) }}
                        ...
                        <div className="flex flex-col items-center justify-center w-12 h-12 rounded-xl shrink-0" style={{ backgroundColor: color + '15' }}>
                          <span className="text-xs font-bold leading-none" style={{ color }}>{time}</span>
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="m-0 text-sm font-semibold text-[#1E293B]">{a.notes || 'Appointment'}</p>
                          <p className="m-0 text-xs text-[#64748B]">
                            {date}{a.doctor?.user?.username ? ` · ${a.doctor.user.username}` : ''}
                          </p>
                        </div>
        ...
        <Modal open={detailOpen} onClose={() => setDetailOpen(false)}
            ...
            <>
              {(selected.status === 'PENDING' || selected.status === 'CONFIRMED') && (
                <>
                  ...
                  <Button onClick={() => cancel(selected.id)} variant="danger" size="md">Cancel</Button>
                </>
              )}
          ...
          <div className="grid grid-cols-2 gap-3">
            {[['Date', selected.appointmentDate?.split('T')[0]],
              ...
              ['Doctor', selected.doctor?.user?.username ?? '—'],
      ...
      <Modal open={rescheduleOpen} onClose={() => setRescheduleOpen(false)}
        ...
        footer={<><Button variant="ghost" onClick={() => setRescheduleOpen(false)}>Cancel</Button><Button onClick={reschedule} disabled={saving}><CalendarClock size={14} /> Confirm</Button></>}>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Book Appointment"
        subtitle="Weekdays only · 9:00 AM – 6:00 PM"
        footer={<><Button variant="ghost" onClick={() => setModalOpen(false)}>Cancel</Button><Button onClick={book} disabled={saving || doctors.length === 0}>{saving ? 'Booking...' : 'Book'}</Button></>}>
        <div className="flex flex-col gap-1">
          <div className="flex flex-col gap-1.5 mb-4">
            <label className="text-xs font-semibold text-[#1E293B]">Doctor *</label>
            <select value={form.doctorPublicId} onChange={handle('doctorPublicId')}
              ...
              <option value="">{doctors.length === 0 ? 'Loading doctors...' : 'Select a doctor'}</option>
              {doctors.map(d => (
                <option key={d.user.publicId} value={d.user.publicId}>
                  {d.user.username} — {d.specialization}
                </option>
              ))}
            </select>
          </div>
          <div className="flex flex-col gap-1.5 mb-4">
            <label className="text-xs font-semibold text-[#1E293B]">Appointment Type *</label>
          ...
          <div className="grid grid-cols-2 gap-x-4">
            <InputField label="Date *" type="date" value={form.date} onChange={handle('date')} />
            <InputField label="Time *" type="time" value={form.time} onChange={handle('time')} />
          </div>
          <InputField label="Notes" placeholder="Optional notes for your doctor..." value={form.notes} onChange={handle('notes')} />
        </div>
      </Modal>
    </div>
  )
